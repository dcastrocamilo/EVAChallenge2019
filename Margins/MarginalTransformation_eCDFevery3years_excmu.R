...
get.data.i <- function(i){
  ...
  for(j in unique(year)){
    ...
    if(j > min(unique(year)) & j < max(unique(year)))
      w.fun <- ecdf(anom.training[year >= (j-1) & year <= (j+1), i]) # eCDF computed every 3 years at each site {i}
    if(j <= min(unique(year)))
      w.fun <- ecdf(anom.training[year >= (j) & year <= (j+1), i])
    if(j >= max(unique(year)))
      w.fun <- ecdf(anom.training[year >= (j-1) & year <= (j), i])
    ...
    for(k in 1:12){
      ...
      w.x = w.fun(anom.training[month == k & year == j, i])
...
save(out, file = paste0("~/EVAChallenge2019/Margins/OutputsByLocMu/out_Rs=", min(Rs), "-", max(Rs), "_3Years.Rdata"))
print('Outputs saved')
