...
for(j in 1:length(x)){
  ...
  if(year[j] > min(unique(year)) & year[j] < max(unique(year))){
    x. = x[year >= (year[j]-1) & year <= (year[j]+1)]
    w.fun <- ecdf(x.) 
    xedfun = edfun(x.[!is.na(x.)])
  }
    
  if(year[j] <= min(unique(year))){
    x. = x[year >= year[j] & year <= (year[j]+1)]
    w.fun <- ecdf(x.)
    xedfun = edfun(x.[!is.na(x.)])
  }
    
  if(year[j] >= max(unique(year))){
    x. = x[year >= (year[j]-1) & year <= year[j]]
    w.fun <- ecdf(x.)
    xedfun = edfun(x.[!is.na(x.)])
  }
